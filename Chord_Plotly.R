...
dat <- read.csv("C:/users/ahsnjgf7/desktop/matcon.csv")
...
addGaps <- function(theta, gap = 0.09){
...
toAngular <- function(x, rad = 1, gap = 0.09, lower = 0, upper = 2*pi, addgaps = T){
...
description <- paste("This chord diagram shows the inter-relationships between conditions within the Norton Healthcare", 
                     "employee population. Conditions are arranged radially around a circle with the relationships colored",
                     "chords connecting the conditions. The size of the chord represents the size of the population which", 
                     "share the connecting conditions.",sep="<br>") 

# Coordinates for labels
labels <- data.frame(
  "text" = c("<b>Overweight</b>","<b>Obese</b>","<b>Prediabetes</b>","<b>Diabetes</b>","<b>Borderline <br> Hyperlipidemia</b>",
             "<b>Hyperlipidemia</b>","<b>Prehypertension</b>","<b>Hypertension</b>","<b>TobaccoUse</b>") , 
  "x" = c(1.02,0.0,-1.05,-1.12,-0.998,-0.76,0.71,1.08,1.12),
  "y" = c(0.53,1.06,0.45,0.04,-0.57,-0.87,-0.90,-0.42,-0.20))

# Plot using plot_ly()
chord.plot <- plot_ly(width = 850, height = 800) %>%
  ...
  layout(
    ...
    annotations = list(
      list(xref = "paper", yref = "paper",
           ...
           x = 0, y = 1, showarrow = F,
           text = "<b>Employee Health Condition Relationships</b>",
...
plotly_POST(chord.plot, filename = "74")
